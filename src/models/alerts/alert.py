...
class Alert(object):
    ...
    def send(self):
        """
        Use mailgun to send the user an email telling them that we found the alert because the item price has gone down
        :return:
        """
        return requests.post(
            ...
            data={
                ...
                "text": "We've found a deal! ({}). To navigate to the alert, visit{}".format(
                    self.item.url, "http://pricing.yolanda.com/alerts/{}".format(self._id))
            }
        )
    ...
    def find_needing_update(cls, minutes_since_update=AlertConstants.ALERT_TIMEOUT):
        """
        Find in the database the alerts that haven't been checked in last 10 minutes
        :param minutes_since_update:
        :return:
        """
        ...
        return [cls(**elem) for elem in Database.find(AlertConstants.COLLECTION,
                                                      {"last_checked":
                                                           {"$lte": last_updated_limit}, # lte: less than or equal to
                                                       "active": True
                                                       })]
